...
class MobileIdAppendInCookieTest(TestCase, ApplyPatchMixin):
    ...
    def setUp(self):
        ...
        self.mobile_app_id = (self.ios_app_id, self.android_app_id)

        self.get_user_organizations = self.apply_patch(
            'api_client.user_api.get_user_organizations'
        )

        self.get_mobile_app_id = self.apply_patch(
            'accounts.controller.get_mobile_app_id'
        )

    def test_has_mobile_app_id(self):
        """
        Tests append_user_mobile_app_id_cookie helper method when user login
        """
        user_id = 4
        self.get_user_organizations.return_value = [{}]
        self.get_mobile_app_id.return_value = self.mobile_app_id
