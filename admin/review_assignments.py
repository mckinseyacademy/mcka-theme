...
class ReviewAssignmentProcessor(object):
    ...
    def assert_possible(self):
        for wg in self.workgroups:
            possible_reviewers = [u for u in self.user_ids if u not in wg.user_ids]
            if len(possible_reviewers) < self.review_target:
                raise ReviewAssignmentUnattainableError()

        for user_id in self.user_ids:
            possible_workgroups = [wg for wg in self.workgroups if user_id not in wg.user_ids]
    ...
    def distribute_pass(self, workgroups_in_need, user_ids):
        ...
        for workgroup in workgroups_in_need:
            possible_ids = [i for i in user_ids if
                            i not in self.workgroup_reviewers[workgroup.id] and
                            i not in workgroup.user_ids and
                            i not in users_dispensed]
    ...
    def distribute(self, delete_existing=False, *args, **kwargs):
        ...
        workgroups_in_need = [wg for wg in self.workgroups if
                              len(self.workgroup_reviewers[wg.id]) < min(review_threshold, self.review_target)]
        ...
        while review_threshold <= self.review_target or len(workgroups_in_need) > 0:
            ...
            if len(users_available) < 1 or (len(workgroups_in_need) == 1
                                            and len([u for u in users_available if
                                                     u not in workgroups_in_need[0].user_ids]) == 0):
                user_threshold += 1
        ...
        while len(users_in_need) > 0:
            ...
            while len(workgroups_available) < 1:
                user_review_threshold += 1
                workgroups_available = [wg for wg in self.workgroups if
                                        len(self.workgroup_reviewers[wg.id]) < user_review_threshold]
