...
class ReviewAssignmentProcessor(object):
    ...
    def __init__(self, users, workgroups, target = 3):
        ...
        self.reviewer_workgroups = {us.id: [] for us in self.users}

    def assert_possible(self):
        for wg in self.workgroups:
            possible_reviewers = [u for u in self.users if not u.id in wg.users]
            if len(possible_reviewers) < self.target:
                raise ReviewAssignmentUnattainableError()


    def distribute_pass(self, workgroups_in_need, users_available):
        user_ids = [u.id for u in users_available]
    ...
    def distribute(self):
        ...
        while review_threshold <= self.target or len(workgroups_in_need) > 0:
            ...
            users_available = [us for us in self.users if len(self.reviewer_workgroups[us.id]) < user_threshold]

            # only 1 avilable might have only its users available
            if len(workgroups_in_need) < 1 and review_threshold <= self.target:
                review_threshold += 1

            if len(users_available) < 1 or (len(workgroups_in_need) == 1 and len([u for u in users_available if not u.id in workgroups_in_need[0].users]) == 0):
                user_threshold += 1
